...
void draw_spectra_select_fun(int mode_val, int input_FSI, int particle, int p_m)
{
    ...
    if (particle_list.count(particle) > 0)
        std::cout << "Found supported particle. Now continue... with particle=" << particle_list[particle] << std::endl;
    else
    {
    ...
    ofstream logfile;
    logfile.open("./log_spectra.log", std::ios::app);
    logfile << mode_map[mode_val] << "\t" << FSI_map[input_FSI] << "\t" << particle_list[particle] << endl;
    logfile << "Original(Green) mean:" << TH1D_original->GetMean() << endl;
    logfile << pm_list[p_m] << "1 sigma mean:" << TH1D_modified_1->GetMean() << "  " << std::setprecision(3) << 100. * (TH1D_modified_1->GetMean() - TH1D_original->GetMean()) / TH1D_original->GetMean() << "%" << endl;
    logfile << pm_list[p_m] << "2 sigma mean:" << TH1D_modified_2->GetMean() << "  " << std::setprecision(3) << 100. * (TH1D_modified_2->GetMean() - TH1D_original->GetMean()) / TH1D_original->GetMean() << "%" << endl;
    logfile << pm_list[p_m] << "3 sigma mean:" << TH1D_modified_3->GetMean() << "  " << std::setprecision(3) << 100. * (TH1D_modified_3->GetMean() - TH1D_original->GetMean()) / TH1D_original->GetMean() << "%" << endl;
    logfile << endl;
    logfile.close();
    delete canvas_0;
}
